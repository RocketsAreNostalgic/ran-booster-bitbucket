...
?>
		<p><?php esc_html_e( 'The Bitbucket Cloud add-on connects RAN Booster to public and private Bitbucket repositories. It supplies repository discovery, archive downloads, credential checks, signed webhook handling and provider diagnostics while Core remains responsible for package records, secret custody, deployment policy and deployment execution.', 'ran-booster-bitbucket' ); ?></p>
			<p><?php esc_html_e( 'This is trusted credential-bearing provider code. Core binds the provider to the bb namespace; during named provider operations the add-on may request individual Bitbucket credentials from that namespace more than once. It cannot enumerate or read another provider’s credentials, store the plaintext or receive Core’s sidecar, key or container. This supported boundary is not confidentiality from hostile PHP running in the same WordPress process.', 'ran-booster-bitbucket' ); ?></p>
		...
		<p><?php esc_html_e( 'A Transporter Blueprint records the selected package and Bitbucket repository configuration. A file-stored API-token profile is copied only when you explicitly select it for the password-protected Blueprint. Profiles that Booster will remove automatically are unavailable for transfer; provider-native or manually recorded expiry does not itself block transfer, and Transporter does not assess token permissions.', 'ran-booster-bitbucket' ); ?></p>
		<p><?php esc_html_e( 'On the target, keep the compatible Bitbucket add-on active and choose separately for each carried credential: import the copied material, use a saved Bitbucket credential on that site, or leave its packages unchanged. There is no credential or anonymous fallback. Preview writes no credential; Apply revalidates the exact repository before Core re-encrypts an imported credential with the target site’s key.', 'ran-booster-bitbucket' ); ?></p>
		<p><?php esc_html_e( 'Transfer copies a credential. It does not remove, revoke or rotate the source API token, and it cannot reconstruct a source-local automatic-removal date from an older Blueprint V1 archive.', 'ran-booster-bitbucket' ); ?></p>
		...
		<h3><?php esc_html_e( 'Releases and support', 'ran-booster-bitbucket' ); ?></h3>
		<p><?php esc_html_e( 'Updates are supplied as verified GitHub release ZIPs with a matching SHA-256 checksum. Generated source archives are not distributable WordPress packages.', 'ran-booster-bitbucket' ); ?></p>
		<p><?php esc_html_e( 'For ordinary, non-sensitive support, use the repository’s GitHub issue tracker. Never post API tokens, account email addresses, webhook secrets, signed URLs, release assets or customer data in an issue. For a suspected security vulnerability, request a confidential contact route without disclosing the sensitive details.', 'ran-booster-bitbucket' ); ?></p>
