...
that sibling Core repository and is intentionally not used as a fallback.
...
4. The `GITHUB_TOKEN` pull-request Quality run is created but requires approval. Wait instead for the release workflow's automatically runnable dispatch for the exact Release Please head. Exact candidate Git reads use the workflow token only through per-command credential helpers; checkout credentials remain disabled. It validates the bot-owned pull request and its four generated files, builds from that head, installs the ZIP without activating its Core dependency, and reads back the installed version, headers, and bytes. Manual dispatch is not release evidence. The contract test must confirm Provider API 10, Add-on API 16, absence of the retired Logging API and optional release and webhook-management facets, and `ran_booster_documentation_sections_after_provider_bb`.
