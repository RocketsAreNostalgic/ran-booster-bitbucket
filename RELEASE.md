...
`extra.ran-booster-core-certification`. Bitbucket tests load Core's shipped production `autoload.php` and public contracts. The Quality repository lane installs only the exact certified Core checkout's locked production Composer dependencies (`--no-dev`) so those shipped contracts have their runtime dependency graph available; it does not reconstruct Core's development graph or import Core-owned test fixtures.
...
5. Run `composer check` for the Core-independent source-quality contract. Then check out the exact certified Core commit locally, run `composer install --no-dev --no-interaction --prefer-dist --no-progress` in that Core checkout to provide its locked production runtime dependencies, set `RAN_BOOSTER_CORE_PATH` to it and `RAN_BOOSTER_CORE_VENDOR_AUTOLOAD` to its generated `vendor/autoload.php`, and run `composer check:repository` plus the PHPStan pilot from the Bitbucket repository. Do not install Core development dependencies. Then run `composer build:release -- "$(git rev-parse HEAD)"`.
...
`tests/WordPress/bitbucket-installed-proof.sh` is the repeatable installed boundary for step 7. Point it only at an explicitly marked disposable WordPress site and provide the exact Core and Bitbucket archives, their SHA-256 digests, the full Bitbucket source commit, and the Bitbucket version. The driver reads the required Core tag, tag-target commit, and archive-source commit directly from this repository's `extra.ran-booster-core-certification`; optional `RAN_BOOSTER_CORE_TAG`, `RAN_BOOSTER_CORE_COMMIT`, and `RAN_BOOSTER_CORE_ARCHIVE_SOURCE_COMMIT` inputs are accepted only when they exactly match that canonical record. It verifies both archives before installation and verifies the Core archive's own release-provenance marker; no Core source checkout or Core-owned test fixture is accepted as proof. It confirms the installed Bitbucket tree exactly matches the retained ZIP, activates the dependency pair normally, exercises both stored plugin load orders, validates the public provider-registration contract and authenticated-delivery diagnostic, and runs missing/incompatible-Core inertness probes.
