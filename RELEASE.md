...
4. Dispatch the Quality workflow on the Release Please pull-request branch (`gh workflow run quality.yml --ref <release-please-branch>`). This explicit dispatch is required because a pull request maintained with `GITHUB_TOKEN` does not trigger another Actions workflow. Quality must check out the exact certified Core tuple; do not override it with Core `main`. The contract test must confirm Provider API 10, Add-on API 15, absence of the retired Logging API, absence of optional Bitbucket webhook fitness and management capabilities, and `ran_booster_documentation_sections_after_provider_bb`.
