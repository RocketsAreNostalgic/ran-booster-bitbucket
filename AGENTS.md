...
- RAN Booster is required. This add-on supports exactly Provider API `8` and the official-suite Add-on API `14` generation, WordPress 7.0+, and PHP 8.2+; PHP 8.4 is recommended, not required.
- Provider API `8` supplies no logging capability. Diagnostics return only bounded typed results; do not add an add-on-to-Core logging channel or local fallback.
