...
- Credentials come only from Core's provider-bound `ProviderCredentialStore`. Provider API 10 registration supplies that store, Core's provider-bound `AuthenticatedWebhookDeliveryEvidenceReader`, and the bounded `ProviderRegistrationContext`. Bitbucket requires the current three-argument registration shape but does not currently consume the context's artifact-limit policy. The add-on neither stores credentials nor reads Core's sidecar paths, service container, database repositories or test fixtures.
